   ...
   public function absenMasuk($type, $result)
   {
      ...
      switch ($type) {
         case TipeUser::Guru:
            ...
            if ($sudahAbsen) {
               $this->presensiGuruModel->updateAbsenMasuk($sudahAbsen, $time);
            } else {
               $this->presensiGuruModel->absenMasuk($idGuru, $date, $time);
            }
            $messageString = $result['nama_guru'] . ' dengan Kode ' . ($result['kode_guru'] ?? '') . $messageString;
         ...
         case TipeUser::Siswa:
            ...
            if ($sudahAbsen) {
               $this->presensiSiswaModel->updateAbsenMasuk($sudahAbsen, $time);
            } else {
               $this->presensiSiswaModel->absenMasuk($idSiswa, $date, $time, $idKelas);
            }
            $messageString = 'Siswa ' . $result['nama_siswa'] . $messageString;
