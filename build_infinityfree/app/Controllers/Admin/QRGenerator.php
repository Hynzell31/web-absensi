...
class QRGenerator extends BaseController
{
   ...
   public function downloadQrGuru($idGuru = null)
   {
      ...
      try {
         ...
         return $this->response->download(
             $this->generate(
                nama: $guru['nama_guru'],
                nomor: $guru['kode_guru'] ?? $guru['id_guru'],
   ...
   public function downloadQrPetugas($idPetugas = null)
   {
      $petugas = (new \App\Models\PetugasModel())->getPetugasById($idPetugas);
      if (!$petugas) {
         session()->setFlashdata([
      ...
      try {
         ...
         $this->qrCodeFilePath .= 'qr-petugas/';

         if (!file_exists($this->qrCodeFilePath)) {
            mkdir($this->qrCodeFilePath, recursive: true);
         }

         // Petugas unique_code dibuat dari kombinasi username + email
         $uniqueCode = sha1($petugas['username'] . $petugas['email']);
