...
class PresensiSiswaModel extends Model implements PresensiInterface
{
   ...
   public function updateAbsenMasuk(string $id, $time)
   {
      $this->update($id, [
         'jam_masuk' => $time,
         'id_kehadiran' => Kehadiran::Hadir->value,
   ...
   {
      $query = $this->db->table('tb_siswa')
         ->select('tb_siswa.*, tb_kelas.tingkat, tb_kelas.index_kelas, CONCAT(tb_kelas.tingkat, " ", tb_kelas.index_kelas) as nama_kelas, tb_presensi_siswa.id_presensi, tb_presensi_siswa.tanggal, tb_presensi_siswa.jam_masuk, tb_presensi_siswa.jam_keluar, tb_presensi_siswa.id_kehadiran, tb_presensi_siswa.keterangan, tb_kehadiran.kehadiran')
      ...
      return $query->orderBy("tb_kelas.tingkat ASC, tb_kelas.index_kelas ASC, nama_siswa ASC")
         ->get()
         ->getResultArray();
   }
