...
class KelasModel extends BaseModel
{
   ...
   {
      return $this->builder
         ->select('tb_kelas.*, tb_guru.nama_guru as nama_wali_kelas, CONCAT(tb_kelas.tingkat, " ", tb_kelas.index_kelas) as kelas')
   ...
   {
      return $this->select('tb_kelas.*, CONCAT(tb_kelas.tingkat, " ", tb_kelas.index_kelas) as kelas')
         ->findAll();
   }
      ...
      if (!empty($array)) {
         ...
         foreach ($array as $item) {
            if ($i == $index) {
               ...
               if ($exists > 0) {
                  return ['status' => 'duplicate', 'data' => $insertData];
               }
